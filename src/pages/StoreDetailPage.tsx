...
export default function StoreDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-retro-bg">
      <header className="bg-gradient-to-br from-amber-600 via-retro-warning to-retro-gold text-white relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-12 relative">
          ...
          {store?.chain && (
            <span className="inline-block mt-2 text-xs font-medium bg-retro-surface/20 px-2.5 py-1 rounded-none backdrop-blur">
              {store.chain}
            </span>
          )}
        </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-6 pb-12">
        {!isLoading && !isError && totalItems > 0 && (
          <p className="text-sm text-retro-muted mb-4">
            Menampilkan <span className="font-semibold text-retro-body">{totalItems}</span> produk
          </p>
        )}
        ...
        {isLoading && (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 overflow-visible">
            {Array.from({ length: 10 }).map((_, i) => (
              <div key={i} className="bg-retro-surface rounded-none border border-retro-border overflow-hidden animate-pulse">
                <div className="h-1.5 bg-retro-surface-alt" />
                <div className="p-3 space-y-2">
                  <div className="flex items-start gap-2.5">
                    <div className="w-9 h-9 bg-retro-surface-alt rounded-none flex-shrink-0" />
                    <div className="flex-1 space-y-1.5">
                      <div className="h-3.5 bg-retro-surface-alt rounded w-3/4" />
                      <div className="h-2.5 bg-retro-surface rounded w-1/2" />
                    </div>
                  </div>
                  <div className="h-5 bg-retro-surface rounded w-24" />
                </div>
              </div>
            ))}
          </div>
        ...
        {isError && (
          <div className="text-center py-20">
            ...
            <p className="text-retro-muted font-medium">Gagal memuat data</p>
                ...
                <StaggerItem key={good.goodId}>
                  <div className="relative group/correct">
                    ...
                    <button
                      ...
                      className="absolute bottom-2 right-2 opacity-0 group-hover/correct:opacity-100 focus:opacity-100 transition-opacity flex items-center gap-1 px-2.5 py-1.5 rounded-none text-xs font-semibold text-white bg-retro-primary hover:bg-retro-primary"
                    >
        ...
        {!isLoading && !isError && goods.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-retro-muted font-medium">Belum ada produk</p>
            <p className="text-retro-muted text-sm mt-1">Belum ada produk yang tercatat di toko ini</p>
          </div>
        )}
      </main>
    </div>

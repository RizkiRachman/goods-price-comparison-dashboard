  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-800">Satuan</h1>
          <p className="text-sm text-slate-400 mt-0.5">Kelola satuan ukuran produk</p>
        </div>
        <button
          onClick={() => navigate('/admin/units/new')}
          className="flex items-center gap-2 px-4 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-semibold hover:bg-indigo-700 transition active:scale-[0.97]"
        >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
              typeFilter === t
                ...
                : 'bg-white/60 backdrop-blur-sm text-slate-500 border border-slate-200 hover:bg-slate-50'
            }`}
          >
      ...
      {isLoading ? (
        ...
        <DataTable
          ...
          actions={(u) => (
            <div className="flex items-center gap-2">
              <button
                onClick={(e) => { e.stopPropagation(); navigate(`/admin/units/${u.id}/edit`) }}

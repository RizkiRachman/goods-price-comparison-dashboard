      ...
      <div className="px-6 pt-4 pb-2 sm:pt-6">
        ...
        <div className="mt-4 relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-white/60 backdrop-blur-sm border border-slate-200 rounded-xl text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent transition"
          />
        </div>
      </div>
            ...
            {filtered.map((product) => {
              ...
              return (
                <motion.button
                  ...
                  whileHover={{ x: 4 }}
                  whileTap={{ scale: 0.98 }}
                  className="w-full text-left flex items-center gap-3 px-4 py-3 bg-white/60 backdrop-blur-sm border border-white/20 rounded-2xl hover:border-indigo-300 hover:bg-indigo-50/30 transition-colors group"
                >
                  <div className={`w-10 h-10 ${color.tint} backdrop-blur rounded-xl flex items-center justify-center text-lg flex-shrink-0`}>
                    {color.emoji}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      {product.brand && (
                        <span className="text-[10px] font-medium text-slate-400 bg-white/70 backdrop-blur px-2 py-0.5 rounded-full border border-slate-200/50">
                          {product.brand}
                        </span>
                      )}
